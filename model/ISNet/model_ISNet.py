...
class ResidualBlock_WT(nn.Module):
    def __init__(self, in_channels, out_channels, stride, downsample):
        super(ResidualBlock_WT, self).__init__()
        self.body = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),

            WTConv2d(out_channels, out_channels, 3, 1, bias=False),
            nn.BatchNorm2d(out_channels),
        )
        if downsample:
            self.downsample = nn.Sequential(
...
class ResidualBlock_SD(nn.Module):
    def __init__(self, in_channels, out_channels, stride, downsample):
        super(ResidualBlock_SD, self).__init__()
        self.body = nn.Sequential(
            SDifferenceConv(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),
...
class ResidualBlock_PC(nn.Module):
    def __init__(self, in_channels, out_channels, stride, downsample):
        super(ResidualBlock_PC, self).__init__()
        self.body = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),

            PConv(out_channels, out_channels, 3, 1),
            nn.BatchNorm2d(out_channels),
        )
        if downsample:
            self.downsample = nn.Sequential(
...
class ResidualBlock_FD(nn.Module):
    def __init__(self, in_channels, out_channels, stride, downsample):
        super(ResidualBlock_FD, self).__init__()
        self.body = nn.Sequential(
            FDConv(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),
...
class ResidualBlock_Ref(nn.Module):
    def __init__(self, in_channels, out_channels, stride, downsample):
        super(ResidualBlock_Ref, self).__init__()
        self.body = nn.Sequential(
            RepConv(in_channels, out_channels, kernel_size=3, stride=stride, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),
...
class ISNet(nn.Module):
    def __init__(self, mode='test', layer_blocks=[4] * 3, channels=[8, 16, 32, 64], conv='usual'):
        ...
        if conv == 'usual':
            ...
            conv_in_stem = SDifferenceConv(stem_width, stem_width, 3, 1, 1, bias=False)
        elif conv == 'PC':
            conv_in_stem = PConv(stem_width, stem_width, 3, 1)
        elif conv == 'FD':
            conv_in_stem = FDConv(stem_width, stem_width, kernel_size=3, stride=1, padding=1, bias=False)
        elif conv == 'Ref':
            conv_in_stem = RepConv(stem_width, stem_width, kernel_size=3, stride=1, padding=1)
        ...
        if conv == 'usual':
            ...
            if conv == 'Dyf':
                ...
                block_D = ResidualBlock_SD
            elif conv == 'PC':
                block_D = ResidualBlock_PC
            elif conv == 'FD':
                block_D = ResidualBlock_FD
            elif conv == 'Ref':
                block_D = ResidualBlock_Ref
            self.layer1 = self._make_layer(block=block_D, block_num=layer_blocks[0],  # ResidualBlock_D
                                           in_channels=channels[1], out_channels=channels[1], stride=1)
            self.layer2 = self._make_layer(block=block_D, block_num=layer_blocks[1],  # ResidualBlock_D
                                           in_channels=channels[1], out_channels=channels[2], stride=2)
