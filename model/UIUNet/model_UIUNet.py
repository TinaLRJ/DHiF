...
class RSU7(nn.Module):#UNet07DRES(nn.Module):
    ...
    def __init__(self, in_ch=3, mid_ch=12, out_ch=3, conv='usual'):
        super(RSU7,self).__init__()

        self.rebnconvin = REBNCONV(in_ch,out_ch,dirate=1)

        if conv == 'Dual':
            self.rebnconv1 = REBNCONV_D(out_ch,mid_ch,dirate=1)
        elif conv == 'usual':
        ...
        if conv == 'Dual':
            self.rebnconv3 = REBNCONV_D(mid_ch, mid_ch, dirate=1)
        elif conv == 'usual':
        ...
        if conv == 'Dual':
            self.rebnconv5 = REBNCONV_D(mid_ch, mid_ch, dirate=1)
        elif conv == 'usual':
...
class UIUNET(nn.Module):
    ...
    def __init__(self, in_ch=1, out_ch=1,mode='test', conv='usual'):
        super(UIUNET, self).__init__()
        self.mode = mode
        
        self.stage1 = RSU7(in_ch,32,64, conv=conv)
