...
class ConvBNReLU_D(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int, kernel_size: int = 3, dilation: int = 1, conv: str = 'DHiF'):
        ...
        if conv == 'DHiF':
            ...
            self.conv = SDifferenceConv(in_ch, out_ch, kernel_size, padding=padding, dilation=dilation,
                                               bias=False)
        self.bn = nn.BatchNorm2d(out_ch)
        self.relu = nn.ReLU(inplace=True)
...
class RSU_D(nn.Module):
    """ Residual U-block """

    def __init__(self, height: int, in_ch: int, mid_ch: int, out_ch: int, conv: str):
        super().__init__()
        ...
        for i in range(height - 2):
            if i == 2:
                encode_list.append(DownConvBNReLU_D(mid_ch, mid_ch, conv=conv))
            else:
                encode_list.append(DownConvBNReLU(mid_ch, mid_ch))
            decode_list.append(UpConvBNReLU(mid_ch * 2, mid_ch if i < height - 3 else out_ch))
        encode_list.append(ConvBNReLU(mid_ch, mid_ch, dilation=2))
...
class ILNet(nn.Module):
    ...
    def __init__(self, cfg: dict, out_ch: int = 1, rb=True, mode='test', conv='usual'):
        super().__init__()
        ...
        for c in cfg["encode"]:
            ...
            if conv == 'usual':
                ...
                encode_list.append(RSU_D(*c[:4], conv=conv) if c[4] is False else RSU4F(*c[1:4]))
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        # RB
        if self.rb:
            y3 = F.interpolate(self.side543(decode_outputs[2]), size=[h, w], mode='bilinear', align_corners=False)
            y4 = F.interpolate(self.side543(decode_outputs[1]), size=[h, w], mode='bilinear', align_corners=False)
        ...
        if self.training:
            ...
            if self.rb:
                outputs = [x] + loss_outputs
                outputs = [output.sigmoid() for output in outputs]
                return outputs
            else:
...
def ILNet_M(out_ch: int = 1, mode='test', conv='usual'):
    cfg = {
        # height, in_ch, mid_ch, out_ch, RSU4F, side
    ...
    return ILNet(cfg, out_ch, mode=mode, conv=conv)
