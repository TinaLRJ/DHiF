...
class MSHNet(nn.Module):
    def __init__(self, input_channels, block=ResNet, mode='test', conv='usual'):
        ...
        if conv == 'usual':
            ...
            if conv == 'DHiF':
                ...
                ResNet_D = ResNet_Ref
            self.encoder_0 = self._make_layer(param_channels[0], param_channels[0], ResNet_D)
            self.encoder_1 = self._make_layer(param_channels[0], param_channels[1], ResNet_D, param_blocks[0])
            self.encoder_2 = self._make_layer(param_channels[1], param_channels[2], block, param_blocks[1])
    ...
    def forward(self, x, warm_flag):
        ...
        if warm_flag:
            ...
            if self.mode == 'train':
                return [mask0.sigmoid(), mask1.sigmoid(), mask2.sigmoid(), mask3.sigmoid(), output]
            else:
                return output
        else:
            output = self.output_0(x_d0).sigmoid()
            return output
