...
class Res_CBAM_block_WT(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_block_WT, self).__init__()
...
class Res_CBAM_block_SD(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_block_SD, self).__init__()
...
class Res_CBAM_block_PC(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_block_PC, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1) ###
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace = True)
...
class Res_CBAM_block_FD(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_block_FD, self).__init__()
...
class Res_CBAM_block_Ref(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_block_Ref, self).__init__()
...
class Res_CBAM_block2(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Res_CBAM_block2, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1) ###
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace = True)
...
class DNANet(nn.Module):
    def __init__(self, num_classes=1,input_channels=1, block=Res_CBAM_block, num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256], deep_supervision=True, mode='test', conv='usual'):
        ...
        if conv == 'usual':
            ...
            if conv == 'DHiF':
                ...
                block_D = Res_CBAM_block_SD
            elif conv == 'PC':
                block_D = Res_CBAM_block_PC
            elif conv == 'FD':
                block_D = Res_CBAM_block_FD
            elif conv == 'Ref':
                block_D = Res_CBAM_block_Ref
            self.conv0_0 = self._make_layer(block, input_channels, nb_filter[0])
            self.conv1_0 = self._make_layer(block_D, nb_filter[0],  nb_filter[1], num_blocks[0])  # Res_CBAM_block_WT
            self.conv2_0 = self._make_layer(block_D, nb_filter[1],  nb_filter[2], num_blocks[1])  # Res_CBAM_block_WT
            self.conv3_0 = self._make_layer(block, nb_filter[2],  nb_filter[3], num_blocks[2])
